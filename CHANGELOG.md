...
> **目前版本：v1.24.115**（2026-09-20）
> 最新更新：當 IFAlgo 訊號落後官方價格或缺少可驗證資料時，投資組合顯示「AI 中立」預設狀態，不再顯示歷史加碼標籤。

---

## [v1.24.115] - 2026-09-20
### Fixed
- 投資組合 AI 卡片只在 IFAlgo 訊號日期可驗證且不落後官方價格時顯示最新 AI 指標；過期或缺少訊號時改顯示「AI 中立」。
- 中立狀態保留訊號與官方價格日期，以及「預設顯示」說明；不將歷史加碼／出場當作目前建議，也不產生未取得的 AI 箭頭。

## [v1.24.114] - 2026-09-20
### Fixed
- 以個股 IFAlgo 訊號日期對照官方價格日期；落後時顯示「歷史 AI 加碼／出場」與兩個日期，未驗證日期時不顯示現行訊號。
- 過期訊號不再帶出投資組合加碼優先度、趨勢與量化籌碼提示，也不流入交易摘要的現行 AI 建議。
- 投資組合本機與雲端快取加入個股訊號日期及來源；資料落後時頁面顯示警示。
- IFAlgo 4967 原始資料截至 2026-09-11，而官方價格已到 2026-09-18；上游未提供新訊號前，不自行產生 AI 箭頭。

## [v1.24.113] - 2026-09-20
### Fixed
- 修正官方日K補線功能在 Vercel 伺服器端的 TypeScript 模組路徑與舊版 JavaScript 相容性警告。

## [v1.24.112] - 2026-09-20
### Fixed
- 上市與上櫃個股線圖在 IFAlgo 資料停滯時，改由 TWSE／TPEx 官方日K補上後續交易日，不改寫原有 IFAlgo K 線。
- 官方資料缺失或僅補到部分日期時顯示明確警示；AI 進出場箭頭只呈現在有 IFAlgo K 線的日期，避免誤導。
- 線圖資料與訊號陣列保持穩定，減少其他頁面狀態更新造成的線圖重建。

## [v1.24.111] - 2026-09-14
### Fixed
- 市場資料請求加入最多三次自動重試、逾時與並行上限；失敗回應改為 502 並禁止快取。
- 手動刷新會略過底層官方報價快取；觀察名單在非開盤時間也能重新抓取。
- 避免舊觀察請求覆蓋新資料，並移除早於自動重試完成的頁面逾時。
- 刷新開始即鎖定按鈕；空量化資料不再保存為成功快取，也不從缺少交易訊號的公開資料推算訊號。

## [v1.24.110] - 2026-08-10
### Changed
- Investment 的 20 位登入帳號、27 張資料表、8 個 RPC 與 66 個交易附件已遷移至共用 Supabase 專案，並保留設計平台原有資料。
- Vercel 前後端改連共用 Supabase 專案，降低多開一個 Micro Compute 專案的固定月費。
- 密碼更新回跳網址已加入共用 Auth 允許清單；專案切換會使既有登入階段失效一次，使用者重新登入即可。

### Added
- 新增可重跑、含重複帳號 UUID 對應、JSON/Storage 同步與完整數量／外鍵驗證的 Supabase 遷移工具。

## [v1.24.109] - 2026-07-22
### Added
- 管理帳號的每個帳號卡片新增全部期間的已實現損益、已實現報酬率、勝敗筆數、勝率、股票檔數與交易筆數。
- 已實現交易統計採背景分頁載入，提供讀取中與失敗狀態，不阻塞帳號清單或把查詢錯誤誤顯示為零損益。

### Changed
- 交易紀錄頁與管理帳號頁改用共用的已實現損益統計函式，確保損益、勝敗、檔數與筆數口徑一致。

## [v1.24.108] - 2026-07-17
### Changed
- 觀察頁 AI 資料改依常用分類優先載入：先同步「AI進場＋超高度」、「AI進場＋高度」與「出場後再進場」，其餘分類在背景補齊，縮短主要觀察流程的等待時間。
- 保留既有四路並行、單檔逾時處理與完整背景同步；若沒有舊分類快取，仍維持原觀察名單順序。
